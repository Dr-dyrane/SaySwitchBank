...
import BillProviderSelector from "../components/payment/BillProviderSelector";
import ConfirmModal from "../components/payment/ConfirmModal";
import BillServiceBanner from "../components/payment/BillServiceBanner";
...
const BillsPaymentScreen = () => {
  ...
  useEffect(() => {
    const providers = {
      electricity: { id: 'eko', name: 'Eko Electricity', logo: 'flash', type:'electricity' },
      TV: { id: 'dstv', name: 'DSTV', logo: 'tv',type:'TV' },
      water: { id: 'lagos', name: 'Lagos Water Corporation', logo: 'water',type:'water' },
    };
  ...
  useEffect(() => {
    const filtered = transactions
      .filter((t) => t.service_type.toLowerCase().includes(activeTab.toLowerCase()))
      .slice(0, 2);
  ...
  const handleConfirm = () => {
    ...
    setBillNumber("");
    setAmount("");
    showToast(`${activeTab.charAt(0).toUpperCase() + activeTab.slice(1)} Bill Payment Successful!`, "success");
    router.push({ pathname: "transDetails", params: { id: Math.floor(Math.random() * 1000) } });
  };
        ...
        <View>
          ...
          <View className="space-y-4">
            ...
            <View className="space-y-2">
              <Text className="text-gray-600">{activeTab === "TV" ? "Smart Card Number" : "Meter/Bill Number"}</Text>
              <TextInput
                className="bg-slate-50 p-4 rounded-xl"
                value={billNumber}
                onChangeText={setBillNumber}
                placeholder={`Enter ${activeTab === "TV" ? "smart card" : "meter/bill"} number`}
            ...
            <View className="flex-row items-center bg-slate-50 pl-4 rounded-xl">
              ...
              <TouchableOpacity
                ...
                onPress={() => amount && billNumber && setShowConfirmModal(true)}
                disabled={!amount || !billNumber}
              >
                <Text
                  className={
                    amount && billNumber ? "text-white font-bold" : "text-gray-200 font-bold"
                  }
                >
                  {amount && billNumber
